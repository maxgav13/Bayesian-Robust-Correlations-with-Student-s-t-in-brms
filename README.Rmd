...
Before we jump into using `family = student`, we should talk a bit about $\nu$. This is our new parameter which is set to infinity when using the Gaussian likelihood. $\nu$ is bound at zero but, as discussed in Baez-Ortega's blog, is nonsensical when below 1. As it turns out, $\nu$ is constrained to equal or greater than 1 in brms. The [Stan team currently recommends the gamma(2, 0.1) prior for $\nu$](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations), which is also the current brms default. This is what that looks like:
...
So gamma(2, 0.1) gently pushed the $\nu$ posterior toward low values, but it’s gently-sloping right tail allows for higher values to emerge.
...
Whoa, look at that correlation, `rescore(x,y)`! It’s right about what we’d hope for. Sure, it's not a perfect -0.95, but that's way way way better than `r posterior_summary(f1)["rescor__x__y", 1] %>% round(2)`.
